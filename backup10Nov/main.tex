...
\and
\IEEEauthorblockN{Dr. Ritesh Adjoodha}
\IEEEauthorblockA{\textit{School of Computer Science and Applied Mathematics} \\
\textit{The Universiy of the Witwatersrand}\\
Johannesburg, South Africa \\
...
%The development of NIDS using ML has shown promising results but researchers agree that due to the increasing volume and complexity of network data, the incorporation of DL techniques is required to develop accurate, real-time intrusion detection systems \cite{b4}. Although research into the application of DL methods for intrusion detection is still in its early stages, a number of approaches have already been proposed.\\ %
...
% \noindet Table 1 outlines some of the 

% \noindent Although research into the application of DL methods for network intrusion detection is still in its early stages (Ahmad), several researchers have already begun experimentally evaluating DL algorithms in the netwrok intrusion domain. Table 1 outlines some of these methods and the performance metrics achieved, the results in the table are for five-class classification as the records in the most popular NIDS datasets are classified as either benign or as one of four attack types; User to Remote, Denial of Service , Probe or Remote to Local.\\ 
